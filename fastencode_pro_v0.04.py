#!/usr/bin/env python3
"""
FastEncode Pro - Timeline Edition v0.04
GPU-Accelerated Video Editor with Accessibility Focus
CONSTANT BITRATE ENCODING - TIMELINE-AWARE FRAME-BY-FRAME RENDERING

v0.4.0 Changes:
- Complete rewrite of timeline export rendering engine
- Frame-by-frame timeline-aware rendering (no concat shortcuts)
- Proper timeline time model with deterministic frame scheduling
- Support for gaps, overlaps, and mixed codecs/resolutions
- Hardware decode/encode with NVENC/NVDEC support
- Safe cancellation and resource cleanup
"""
...
class TimelineRenderingEngine:
    """
    Optimized timeline rendering engine v0.6.0.
    
    Key optimization: -ss BEFORE -i in FFmpeg command.
    This is the ONLY change needed for 10x speedup.
    
    NO async. NO buffering. NO complexity.
    Just fix the actual bottleneck.
    """
    
    def __init__(self, timeline_widget, settings, output_path,
        ...
        self.playhead = playhead_callback  # NEW: For updating timeline playhead during render
        
        self.should_stop = False
        self.encoder_process = None
        self.decoders = {}  # Cache decoders per file
    ...
    def get_clip_at_timeline_time(self, timeline_time):
        ...
        for clip in self.timeline.clips:
            if clip.start_time <= timeline_time < clip.get_end_time():
                clip_time = clip.timeline_time_to_clip_time(timeline_time)
                candidates.append((clip, clip_time))
        
        if not candidates:
            return None, None
        
        candidates.sort(key=lambda x: x[0].track, reverse=True)
        return candidates[0]
    ...
    def render(self):
        """
        Simple synchronous rendering with OPTIMIZED frame extraction.
        
        The ONLY change from v0.4.0: Use -ss BEFORE -i in FFmpeg.
        This one change gives 10x speedup. No other changes needed.
        """
        try:
            self.log("=== OPTIMIZED TIMELINE RENDERING ENGINE v0.6.0 ===")
            self.log(f"Output: {self.output_path}")
            self.log("Optimization: Fast seeking with -ss before -i")
            ...
            source_width, source_height, _ = self.get_video_metadata(sorted_clips[0].file_path)
            ...
            self.log(f"\nTimeline: {timeline_duration:.2f}s @ {timeline_fps} fps")
            self.log(f"Resolution: {export_width}x{export_height}")
            self.log(f"Total frames: {total_frames}")
            
            # Start encoder
            if not self._start_encoder(timeline_fps, export_width, export_height):
            ...
            for frame_num in range(total_frames):
                ...
                timeline_time = frame_num / timeline_fps
                
                # Find clip
                clip, clip_time = self.get_clip_at_timeline_time(timeline_time)
                
                if clip is None:
                    # Black frame
                    frame_data = self._create_black_frame(export_width, export_height)
                else:
                    # Decode frame - OPTIMIZED
                    frame_data = self._decode_frame_optimized(clip, clip_time, export_width, export_height)
                ...
                try:
                    ...
                    self.log(f"\nEncoder write error: {e}")
                    return False, "Encoder error"
                ...
                if frames_rendered >= last_log_frame + int(timeline_fps):  # Log every second
                    elapsed = time.time() - start_time
                    fps_actual = frames_rendered / elapsed if elapsed > 0 else 0
                    progress_pct = int((frames_rendered / total_frames) * 100)
                    
                    self.progress(progress_pct)
                    self.log(f"  Frame {frames_rendered}/{total_frames} ({progress_pct}%) | {fps_actual:.1f} fps | timeline: {timeline_time:.1f}s")
                    
                    last_log_frame = frames_rendered
            ...
            if self.encoder_process.returncode != 0:
                return False, "Encoder failed"
            
            # Success
            elapsed = time.time() - start_time
            fps_avg = total_frames / elapsed if elapsed > 0 else 0
            file_size_gb = os.path.getsize(self.output_path) / (1024**3)
            
            self.log(f"\n✅ RENDER COMPLETE")
            self.log(f"Total time: {elapsed:.1f}s")
            self.log(f"Average speed: {fps_avg:.1f} fps ({fps_avg/timeline_fps:.2f}x realtime)")
            self.log(f"File size: {file_size_gb:.2f} GB")
            
            self.progress(100)
            return True, f"Rendered {frames_rendered} frames @ {fps_avg:.1f} fps\nSize: {file_size_gb:.2f} GB"
    ...
    def _decode_frame_optimized(self, clip, clip_time, width, height):
        """
        OPTIMIZED frame decoding.
        
        KEY OPTIMIZATION: -ss BEFORE -i
        This uses input seeking instead of output seeking.
        10-100x faster than -ss after -i.
        """
        try:
            ...
            cmd = [
                'ffmpeg',
                '-ss', str(clip_time),  # CRITICAL: BEFORE -i = fast seek
                '-i', clip.file_path,
                '-vframes', '1',
                '-vf', f'scale={width}:{height}:flags={scale_algo},format=yuv420p',
                '-f', 'rawvideo',
                '-pix_fmt', 'yuv420p',
                '-'
            ]
    ...
    def _create_black_frame(self, width, height):
        ...
        return bytes([16] * y_size) + bytes([128] * uv_size) + bytes([128] * uv_size)
    
    def _start_encoder(self, fps, width, height):
        try:
